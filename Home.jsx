/* global React, IconLine, Button, Wordmark, BotanicalLeaf, useInView, CountUp, Stars, TopNav, ChatFab */

const HomeScreen = ({ onOpenKit, onCart, onChat, screenRef }) => {
  const { useState, useEffect, useRef } = React;
  const kits = window.LIORA_KITS;
  const trust = window.LIORA_TRUST;
  const steps = window.LIORA_STEPS;
  const stats = window.LIORA_STATS;
  const reviews = window.LIORA_REVIEWS;

  // Logo intro animation: fades in center → slides up to nav
  ...
  useEffect(() => {
    ...
    const onScroll = () => {
      ...
      leafRef.current.style.transform = `translateY(${el.scrollTop * 0.3}px)`;
    };
    el.addEventListener("scroll", onScroll, { passive: true });
    return () => el.removeEventListener("scroll", onScroll);
  }, [screenRef]);

  // Stats viewport trigger
  const [statsRef, statsIn] = useInView({ threshold: 0.4 });

  // Kit shelf — track active card for scale 1.05
    ...
    <>
      ...
      {!introDone && (
        <div style={{
          ...
          <div style={{
            animation: "liora-logo-intro 900ms var(--ease-out) forwards",
          }}>
      ...
      <TopNav scrollRef={screenRef} onCart={onCart} onMyKits={() => alert("Mis kits")}/>

      {/* HERO */}
      <section style={{ position: "relative", padding: "16px 24px 40px", overflow: "hidden" }}>
        ...
        <h1 style={{
          ...
          {headlineWords.map((w, i) => (
            <span key={i} className="fade-up" style={{
              ...
              fontStyle: typeof w !== "string" ? "italic" : "normal",
            }}>
              {w}
            </span>
          ))}
        ...
        <p className="lead fade-up" style={{
          ...
          animationDelay: "1300ms", fontSize: 16, lineHeight: 1.55,
          color: "var(--fg-muted)",
        }}>
          Kits completos de autocuidado con productos seleccionados e instrucciones claras
          para saber qué usar, cómo usarlo y cuándo pedir ayuda.
        </p>

        <div className="fade-up" style={{ display: "flex", flexDirection: "column", gap: 12, marginTop: 24, animationDelay: "1450ms" }}>
          <button className="btn btn-primary" onClick={() => onOpenKit && screenRef.current.scrollTo({ top: 740, behavior: "smooth" })}>
            Encontrar mi kit <IconLine name="arrow" size={16} stroke="currentColor"/>
          </button>
          <button className="btn btn-link" style={{ alignSelf: "flex-start" }}
            onClick={() => screenRef.current.scrollTo({ top: 740, behavior: "smooth" })}>
            Ver todos los kits →
          </button>
        </div>

        {/* Hero product image — slides in from offscreen */}
        <div className="photo warm" style={{
          marginTop: 28, height: 220, borderRadius: 0,
          animation: "liora-hero-product 700ms var(--ease-out) 1100ms both",
        }}>
          <div className="label">Kit Bienestar · Edición Otoño</div>
        </div>
        <style>{`
          @keyframes liora-hero-product {
        ...
        <div style={{
          ...
          marginTop: 32, opacity: 0.65,
          animation: "liora-pulse 2.4s ease-in-out infinite",
        }}>
      ...
      <section style={{ background: "var(--liora-verde-salvia)", color: "var(--liora-cream)", padding: "32px 24px" }}>
        ...
        <h3 style={{
          ...
          lineHeight: 1.2, letterSpacing: "-0.01em", marginTop: 8,
        ...
        <form
          ...
          style={{
            ...
            background: "rgba(250,250,245,0.10)",
          ...
          <input
            ...
            style={{
              flex: 1, background: "transparent", border: 0, color: "var(--liora-cream)",
              fontFamily: "var(--font-body)", fontSize: 14, padding: "10px 12px", outline: "none",
            }}
          />
          <button type="submit" className="btn btn-warm" style={{
            ...
            {submitted ? (
              <><IconLine name="check" size={16} stroke="currentColor"/> ¡Listo!</>
            ) : "Quiero el 15%"}
          </button>
        </form>
        {submitted && (
          <div className="fade-in" style={{
            marginTop: 12, fontSize: 13, color: "rgba(250,250,245,0.85)",
          }}>
      ...
      <section style={{ paddingTop: 48 }}>
        ...
        <div ref={shelfRef} className="snap-x" style={{ marginTop: 20 }}>
          {kits.map((k, i) => (
            <ShelfCard key={k.id} kit={k} index={i} active={i === activeKit} onOpen={onOpenKit}/>
          ))}
        </div>
        ...
        <div style={{ display: "flex", gap: 6, justifyContent: "center", marginTop: 8 }}>
          {kits.map((_, i) => (
            <span key={i} style={{
              width: i === activeKit ? 18 : 6, height: 6, borderRadius: 999,
              background: i === activeKit ? "var(--liora-verde-salvia)" : "var(--liora-gris-medio)",
      ...
      {/* STATS — count up */}
      <section ref={statsRef} className="section--tight" style={{
        background: "var(--liora-verde-claro)", padding: "40px 24px",
        margin: "16px 16px", borderRadius: 8,
      }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
          {stats.map((s, i) => (
            <div key={i} style={{ display: "flex", alignItems: "baseline", gap: 16, paddingBottom: 16, borderBottom: i < stats.length - 1 ? "1px solid rgba(47,58,53,0.15)" : "0" }}>
              <div style={{
                fontFamily: "var(--font-display)", fontWeight: 500, fontSize: 56,
                lineHeight: 1, letterSpacing: "-0.02em", color: "var(--liora-verde-profundo)",
                minWidth: 110,
              }}>
        ...
        <div className="photo dark" style={{ position: "relative", height: 220, cursor: "pointer" }}>
          <div style={{
            ...
            <div style={{
              ...
              background: "rgba(250,250,245,0.92)", display: "inline-flex",
              alignItems: "center", justifyContent: "center",
              boxShadow: "0 6px 20px rgba(47,58,53,0.25)",
            }}>
      ...
      <footer style={{ background: "var(--liora-verde-profundo)", color: "var(--liora-cream)", padding: "48px 24px 80px", textAlign: "center" }}>
        ...
        <div style={{ fontFamily: "var(--font-body)", fontSize: 11, letterSpacing: "0.20em", textTransform: "uppercase", marginTop: 8, opacity: 0.8 }}>
          Cuidado integral
        </div>
        <p style={{ fontFamily: "var(--font-display)", fontStyle: "italic", fontSize: 16, marginTop: 20, opacity: 0.85 }}>
          Tu bienestar, nuestra guía.
        </p>
...
function ShelfCard({ kit, index, active, onOpen }) {
  ...
  return (
    <article
      ref={ref}
      className={"shelf-card" + (seen ? " is-seen" : "")}
      ...
      <div className={photoCls} data-kit-photo={kit.id} style={{ height: 240 }}>
        <div className="label">{kit.name.replace("Kit ", "")}</div>
      </div>
      <div style={{ padding: 18, display: "flex", flexDirection: "column", gap: 8 }}>
        <div className="eyebrow-cap">{kit.eyebrow}</div>
        <h3 data-kit-title={kit.id} style={{ fontFamily: "var(--font-display)", fontWeight: 600, fontSize: 20, lineHeight: 1.2, margin: 0 }}>{kit.name}</h3>
        <p style={{ fontFamily: "var(--font-body)", fontSize: 12, lineHeight: 1.55, color: "var(--fg-muted)", margin: 0 }}>{kit.short}</p>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 4 }}>
          <div style={{ fontFamily: "var(--font-body)", fontSize: 15, fontWeight: 500 }}>{window.formatPrice(kit.price)}</div>
  ...
  return (
    <div ref={ref} style={{
      ...
      <div style={{
        ...
        background: "var(--liora-verde-claro)",
...
function ReviewCard({ r }) {
  ...
  return (
    <article style={{
      width: 280, background: "var(--liora-cream)",
      ...
      <p style={{
        ...
        fontSize: 17, lineHeight: 1.4, margin: 0,
      }}>"{r.quote}"</p>
