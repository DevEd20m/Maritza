...
const CONTAINER_BG = { sage: '#A8BBAF', warm: '#F7EDE3', deep: '#4E5A52', arena: '#F2E9DB' }

export function IconLine({ name, size = 24, stroke = 'currentColor', container = null, containerSize = 48 }) {
  const props = {
    width: size, height: size, viewBox: '0 0 24 24', fill: 'none',
    stroke, strokeWidth: 1.5, strokeLinecap: 'round', strokeLinejoin: 'round',
  }
  const icon = <svg {...props}>{ICONS[name] ?? ICONS.leaf}</svg>

  if (!container) return icon

  const bg = CONTAINER_BG[container] ?? '#A8BBAF'
  const ink = container === 'deep' ? '#FAFAF5' : '#111312'
  return (
    <div style={{
      ...
      <svg {...props} stroke={ink}>{ICONS[name] ?? ICONS.leaf}</svg>
    </div>
  )
}

export function Wordmark({ size = 28, color = '#111312' }) {
  const fs = size
  const dotSize = fs * 0.10
  const circleSize = fs * 0.85
  return (
    <div style={{
      display: 'inline-flex', alignItems: 'center', gap: fs * 0.18,
      fontFamily: 'var(--font-display)', fontWeight: 500,
      fontSize: fs, letterSpacing: '0.04em', color, lineHeight: 1,
    }}>
      <span>L</span>
      <span>I</span>
      <span style={{ position: 'relative', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', width: circleSize, height: circleSize }}>
        <span style={{
          position: 'absolute', top: -dotSize * 1.0, left: '50%',
          transform: 'translateX(-50%)', width: dotSize, height: dotSize,
          borderRadius: '50%', background: color,
        }} />
        <svg width={circleSize} height={circleSize} viewBox="0 0 24 24" fill="none"
...
export function LogoLockup({ size = 28, color = '#111312', showTagline = true, align = 'center' }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: align === 'center' ? 'center' : 'flex-start', gap: 8 }}>
      ...
      {showTagline && (
        <div style={{ fontFamily: 'var(--font-body)', fontSize: 10, letterSpacing: '0.20em', textTransform: 'uppercase', color, opacity: 0.85 }}>
          Cuidado integral
        </div>
      )}
    </div>
...
export function Button({ variant = 'primary', icon, iconRight, children, onClick, fullWidth, type = 'button', style }) {
  return (
    <button type={type} className={`btn btn-${variant}`} onClick={onClick}
      style={{ width: fullWidth ? '100%' : 'auto', justifyContent: 'center', ...style }}>
