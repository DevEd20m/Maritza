...
import { Wordmark, IconLine } from './Primitives'
...
export function CountUp({ to, suffix = '', decimals = 0, durationMs = 1200, start = false }) {
...
export function BotanicalLeaf({ width = 220, opacity = 0.55, style = {} }) {
  return (
...
export function TopNav({ scrollRef, onCart, onMyKits, dark = false, transparent = false, transparentUntil = 24 }) {
    ...
    <span className="stars" aria-label={`${value} de 5`}>
      {Array.from({ length: 5 }).map((_, i) => (
        <svg key={i} width={size} height={size} viewBox="0 0 24 24"
          fill={i < value ? 'currentColor' : 'none'} stroke="currentColor" strokeWidth="1.4">
